"""Handles the actual resume files: upload to Supabase Storage (so you can
always download the original), plus extract -> chunk -> embed -> store in
`resume_chunks` for RAG search/chat.
"""
...
def upload_resume(
    ...
    # 2. Upload the raw file to Storage so the original is always downloadable
    ...
    # 3. Create the application row (or upsert if this exact application already exists)
    record = {
        "company": company,
        "role": role,
        "url": url or None,
...
def get_download_url(storage_path: str, expires_in: int = 3600) -> str:
    ...
    return resp["signedURL"] if "signedURL" in resp else resp.get("signed_url", "")


def list_resumes() -> pd.DataFrame:
    """All applications that have an actual resume file attached, for the
    'browse & download' view — searchable by filename or date in the UI."""
    client = get_client()
    rows = (
        client.table(APPLICATIONS_TABLE)
        .select("*")
